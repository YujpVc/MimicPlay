...
from robomimic.envs.env_fairino import FairinoEnv, DefaultEnvConfig
...
class DataCollector:
    ...
    def _start_recording(self):
        ...
        obs, info = self.env.reset(joint_reset=True)
    ...
    def _record_step(self, action: np.ndarray, obs, info):
        ...
        # Structure: {"state": {"tcp_pose": [7], "gripper_pose": [2], "joint_pose": [7]}, "images": {...}}
        
        # Extract robot state components from nested structure
        state_dict = obs.get('state', {})
        joints = state_dict.get('joint_pose', np.zeros(6))     # Joint angles in radians (Fairino FR5 is 6-axis)
        tcp_pose = state_dict.get('tcp_pose', np.zeros(7))     # TCP pose: [x, y, z, qx, qy, qz, qw]
        gripper_raw = state_dict.get('gripper_pose', np.zeros(1))  # Gripper position
...
def main():
    ...
    env = FairinoEnv(hz=20, fake_env=False, config=env_config)
    print("    Fairino Ready.")
    ...
    expert = ForceDimensionExpert(
        ...
        max_pos_action=env_config.MAX_POS_DELTA,  # Match env action range
        max_rot_action=env_config.MAX_ROT_DELTA   # Match env action range
    )
    print("    Force Dimension Ready (with rotation smoothing).")
    print(f"    Action range: pos=±{env_config.MAX_POS_DELTA}m, rot=±{env_config.MAX_ROT_DELTA}rad")
