...
class ForceDimensionExpert:
    def __init__(self, device_id=0, scale_pos=0.5, scale_rot=1.0,
        ...
        Args:
            ...
            rot_delta_gain: Gain applied to per-step rotation delta to make
                           slow motions produce visible output.
        ...
        self.rot_delta_gain = rot_delta_gain
        ...
        with self.lock:
            ...
            self.rot_prev = R.from_matrix(self.latest_rot.copy())
    ...
    def get_action(self, obs=None):
        ...
        # --- ROTATION MAPPING (RELATIVE DELTA) ---
        # 1. Get current absolute rotation
        quat_curr = R.from_matrix(mat_curr)
        
        # 2. Check and initialize rot_prev if needed
        if not hasattr(self, 'rot_prev'):
             self.rot_prev = quat_curr

        # 3. Calculate Difference from PREVIOUS to CURRENT
        # R_diff = R_curr * inv(R_prev)
        # This gives the incremental rotation change
        R_diff = quat_curr * self.rot_prev.inv()
        
        # 4. Update rot_prev for next step
        self.rot_prev = quat_curr
        
        # 5. Convert to Euler angles
        raw_euler = R_diff.as_euler('xyz') * self.scale_rot * self.rot_delta_gain
        
        # 6. Apply Smoothing if enabled (Low-pass filter on the output angles)
        if self.smooth_rot:
