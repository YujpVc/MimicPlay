...
参数默认从 mimicplay/configs/highlevel_260301.json 读取（可通过 --config 覆盖）。

使用示例:

  python scripts/vis_highlevel_planner.py \\
    --config mimicplay/configs/highlevel_260301.json \\
    --agent /path/to/model_best.pth \\
    --video_prompt /path/to/demo.hdf5 \\
    --output training_result/highlevel_vis.mp4
"""
...
def visualize_highlevel_planner(args):
    ...
    # 相机参数（与 show_future_traj / vis_planner_video 一致）
...
if __name__ == "__main__":
    ...
    default_config = os.path.join(root, "configs", "highlevel_260301.json")
    ...
    parser.add_argument(
        "--video_prompt",
        type=str,
        default="/home/yujp/Genesis/scripts/XBOX_control/dataCollect/demo/auto_demo20260301155601/image_demo_local.hdf5",
        help="HDF5 演示数据路径（含 agentview_image、robot0_eef_pos）",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=os.path.join(root, "training_result", "highlevel_vis.mp4"),
        help="输出视频路径",
    )
