...
class GenesisEnvWrapper(EB.EnvBase):
    ...
    def __init__(self, env_name=None, env_config=None, camera_width=84, camera_height=84, **kwargs):
        ...
        self.shelf = self.scene.add_entity(
            gs.morphs.MJCF(
                ...
                pos=(0.7, 0.65, 0.88), quat=(0, 0, 0, 1), scale=1,
            )
        )
        ...
        self.book = self.scene.add_entity(
            gs.morphs.Mesh(
                file="/home/yujp/Genesis/my_models/turbosquid_objects/black_book/black_book.obj",
                scale=0.3, pos=(0.7, 0.85, 0.88), quat=(0.7, 0, 0, 0.7),
            )
        )
        self.juice = self.scene.add_entity(
            gs.morphs.Mesh(
                file="/home/yujp/Genesis/my_models/stable_hope_objects/orange_juice/textured.obj",
                pos=(0.75, 1.15, 1.0), quat=(0, 0, 0.707, 0.707), scale=0.0075
            )
        )
        self.wine_bottle = self.scene.add_entity(
            gs.morphs.Mesh(
                file="/home/yujp/Genesis/my_models/turbosquid_objects/wine_bottle/wine_bottle_red_main.obj",
                pos=(0.75, 1.0, 0.88), scale=0.7
            )
        )
        ...
        self.movable_objects = [
            MovableObject(self.book, [0.7, 0.85, 0.88], [0.7, 0, 0, 0.7]),
            MovableObject(self.juice, [0.75, 1.15, 1.0], [0, 0, 0.707, 0.707]),
            MovableObject(self.wine_bottle, [0.75, 1.0, 0.88], [1, 0, 0, 0])
        ]

        # 6. 初始化控制状态 (与采集脚本一致)
        self.initial_pos = np.array([0.65, 1.0, 1.15])
        self.initial_quat = np.array([0, 1, 0, 0])
        self.current_pos = np.copy(self.initial_pos)
        self.current_quat = np.copy(self.initial_quat)
    ...
    def get_observation(self):
        ...
        # 更新手腕摄像头
        wrist_cam_pos = gripper_cam_pos + y_dir * 0.05
        wrist_cam_lookat = wrist_cam_pos + z_dir * 0.5
